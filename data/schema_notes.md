...
| `module_images` / `图片证据` | 当前分析区块的数据图片、图表、流程图、思维导图和截图证据 |
...
前端工作台支持在多个分析区块下上传图片，并写入对应区块的 `图片证据` 字段。适用位置包括产品基础信息、产品定位分析、核心玩法拆解、功能模块拆解、数据指标体系、优化建议和自定义扩展章节。
...
功能模块拆解:
  - 模块名称: "新手引导"
    图片证据:
      - 标题: "新手漏斗截图"
        说明: "展示启动、首关开始、首关完成、第二关开始之间的转化。"
        文件名: "tutorial-funnel.png"
        图片数据: "data:image/png;base64,..."

数据指标体系:
  图片证据:
    - 标题: "指标体系脑图"
      说明: "展示新增、活跃、留存、付费、广告和用户分层指标之间的关系。"
      文件名: "metrics-map.png"
      图片数据: "data:image/png;base64,..."
```
...
- `图片数据` 可以是前端上传后生成的 data URL，也可以是远程图片 URL。
- 如果要放到 GitHub Markdown 长期展示，建议后续把图片文件单独放到仓库的 `assets/` 目录，再把 `图片数据` 替换为相对路径。
- 纯前端自动保存使用浏览器本地存储，图片太大会占用较多空间，建议单张小于 2MB。
